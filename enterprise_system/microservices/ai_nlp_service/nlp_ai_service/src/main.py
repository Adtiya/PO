...
try:
    from validation import (
        NLPAnalysisRequest, TextClassificationRequest, APIResponse, ErrorResponse,
        validate_json, ErrorHandler, RequestValidator
    )
except ImportError:
    # Fallback if validation module not available
    print("Warning: Validation module not available, using basic validation")
    validate_json = lambda schema: lambda func: func
    
    class ErrorHandler:
        @staticmethod
        def handle_validation_error(error): return {"error": str(error)}, 400
        @staticmethod
        def handle_internal_error(error): return {"error": "Internal error"}, 500

# Import monitoring system
try:
    from monitoring import setup_monitoring, StructuredLogger
...
SECRET_KEY = os.getenv('AI_NLP_SERVICE_SECRET')
if not SECRET_KEY:
    raise ValueError("AI_NLP_SERVICE_SECRET environment variable is required")
...
if MONITORING_AVAILABLE:
    health_checker, logger = setup_monitoring(
        app, 
        service_name="AI_NLP_Service",
        db_connection=db,
        external_services=["https://api.openai.com/v1/models"]  # Check OpenAI API
    )
else:
    logger = StructuredLogger("AI_NLP_Service")
...
@validate_json(NLPAnalysisRequest)
def analyze_sentiment(validated_data=None):
    """Analyze sentiment of provided text"""
    try:
        # Use validated data if available, otherwise fall back to basic validation
        if validated_data:
        ...
        error_response, status_code = ErrorHandler.handle_internal_error(e)
...
@validate_json(TextClassificationRequest)
def classify_text(validated_data=None):
